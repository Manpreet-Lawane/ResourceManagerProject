    ...
    def get(self, request):
        ...
        try:
            ...
            column_mapping = {
                ...
                'Wage Related Costs for Interns and Residents': 'WageRelatedCostInternResidents',
    ...
    def post(self, request):
        ...
        for raw_data in raw_data_list:
            ...
            try:
                ...
                RecordNumberInstance = RawData.objects.get(RecordNumber=raw_data.RecordNumber)
                expense_data = {
                    "RecordNumber": RecordNumberInstance,
                ...
                pass
        return Response({"message": "Data processed and saved to Hospitals Expenses Model"},
                        status=status.HTTP_201_CREATED)


class GetExpense(views.APIView):   #### Change view typwe to regular func and change func
    def post(self, request):
        request_hospital_id = request.data.get('HospitalID')
        context = {}

        if Hospitals.objects.filter(HospitalID=request_hospital_id):
            print("Hospital found")
            hospital = Hospitals.objects.get(HospitalID=request_hospital_id)
            if HospitalExpenses.objects.filter(HospitalID=hospital):
                ...
                return context  # Response({"message": "Available", "context": context}, status=status.HTTP_200_OK)
...
class ExpensesGraph(views.APIView):
    def post(self, request):
        expense_data = GetExpense(request)
        expense_df = pd.DataFrame(expense_data, index=[0])
        plt.figure(figsize=(8, 8))
        plt.title("expenses for")
        return Response({"image": 'abc.jpg'}, status=status.HTTP_200_OK)
